...
* 五、IDEA的版本关系
  ...
  * 在`C：`盘符中会自动生成配置目录等
    ...
    * IDEA 的Module(模块) ≈ Eclipse 的Project![IDEA的Module](C:\Users\Kay\Documents\Notes\photos\IDEA\IDEA的Module.png)
...
* private static final **`prsf`** ==当定义了一个private私有的变量在Module(模块)中，project(工程)可以使用Module(模块)下的私有属性，不过需要在iml文件中配置，快捷键为 `alt + enter`  ==Add dependency on module 'xxx'==![](C:\Users\Kay\Documents\Notes\photos\IDEA\Module增加依赖.png)
